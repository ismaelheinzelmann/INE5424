...
void TypeUtils::uintToBytes(unsigned int value, std::vector<unsigned char> *bytes) {
	(*bytes)[3] = static_cast<unsigned char>(value & 0xFF); // Extract least significant byte
	(*bytes)[2] = static_cast<unsigned char>((value >> 8) & 0xFF); // Extract second byte
	(*bytes)[1] = static_cast<unsigned char>((value >> 16) & 0xFF); // Extract third byte
	(*bytes)[0] = static_cast<unsigned char>((value >> 24) & 0xFF); // Extract most significant byte
}
